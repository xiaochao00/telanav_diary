...
class DBInfo(object):
    ...
    def sort_list(db_info_list):
        db_info_list.sort(key=lambda db_info: db_info.version, reversed=False)
        db_info_list.sort(key=lambda db_info: db_info.region, reversed=False)
        db_info_list.sort(key=lambda db_info: db_info.database_name[:6], reversed=False)
