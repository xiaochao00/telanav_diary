...
CSV_SEP           = '`'
LF                = '\n'
CATEGORY_CAMERA       = "9701"
CATEGORY_CHECK_SPEED  = "9705"
CATEGORY_SCHOOL_POINT = "9721"
...
xscp.drivingdirecti_c_linkheading, \
xscp.drivingdirect_cs_linkheading, \
xscp.drivingdirect_sp_linkheading \
FROM \
...
WHERE xscp.countrycode_text IN (%s)"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
    ...
    def __get_link_heading(self,keys,line):
        if self.isNotEmpty(line[10])\
                or (CATEGORY_CAMERA == line[8] and self.isNotEmpty(line[11])) \
                or (CATEGORY_CHECK_SPEED == line[8] and self.isNotEmpty(line[12])) \
                or (CATEGORY_SCHOOL_POINT == line[8] and self.isNotEmpty(line[13])):
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))
