    ...
    def check_sub_directory_name_pattern_list(self, sub_dir_name_pattern_list):
        ...
        for matched_dir in matched_dir_list:
            if not os.listdir(matched_dir):
                Util.print_error("pattern %s matched directory [%s] in directory [%s] should not empty" % (sub_dir_name_pattern_list, matched_dir, self.data_dir))
                return False
        return True
...
class ExtendChecker(object):
    ...
    def generate_report(check_info_list, to_path):
        """Generate check report base on check info list
...
class Util(object):
    ...
    def write_to_js_file(report_path, data_list):
        ...
        :return:
        """
        # 1. generator check_report directory structure
...
if __name__ == '__main__':
    # base_path = '/var/www/check_report/data'
