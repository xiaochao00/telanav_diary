...
class CheckStateInfo(object):
    ...
    DETAIL_COMPONENT_NAME = "component"
    DETAIL_ERROR_MESSAGE_NAME = "error_message"
    DETAIL_STATUS_NAME = "status"

    def __init__(self, region, version, detail_info_list, status):
        self.region = region
    ...
    def transform_to_dic(self):
        ...
        info_dic[self.REGION_NAME] = self.region
        info_dic[self.VERSION_NAME] = self.version

        detail_dic_list = []
        for detail_info in self.detail_info_list:
            detail_dic = OrderedDict()

            detail_dic[self.DETAIL_COMPONENT_NAME] = detail_info.component
            detail_dic[self.DETAIL_STATUS_NAME] = detail_info.status
            detail_dic[self.DETAIL_ERROR_MESSAGE_NAME] = detail_info.error_message

            detail_dic_list.append(detail_dic)

        info_dic[self.DETAILS_NAME] = detail_dic_list
        info_dic[self.STATUS_NAME] = self.status

        return info_dic


class DetailInfo(object):
    def __init__(self, component, status, error_message):
...
class ExtendChecker(object):
    ...
    def check_single_region(region_data_path):
        ...
        if region and version:
            ...
            for failed_component_name, failed_msg in failed_components_check_info_dic.iteritems():
                detail_info = DetailInfo(failed_component_name, CheckStateInfo.CHECK_FAILED_STR, failed_msg)
                detail_info_list.append(detail_info)
            for component_name, msg in success_components_check_info_dic.iteritems():
                detail_info = DetailInfo(component_name, CheckStateInfo.CHECK_SUCCESS_STR, msg)
                detail_info_list.append(detail_info)

            check_state_str = CheckStateInfo.CHECK_FAILED_STR if failed_components_check_info_dic else CheckStateInfo.CHECK_SUCCESS_STR

            return CheckStateInfo(region, version, detail_info_list, check_state_str)
    ...
    def generate_report(check_info_list, to_path):
        ...
        for check_info in check_info_list:
            check_result_dic = check_info.transform_to_dic()
            check_result_data_list.append(check_result_dic)
        # 2.sort data list
        check_result_data_list.sort(key=lambda d: d[CheckStateInfo.VERSION_NAME])
        check_result_data_list.sort(key=lambda d: d[CheckStateInfo.REGION_NAME])
...
class Util(object):
    ...
    def write_to_js_file(report_path, data_list):
        ...
        with open(save_file_path, 'w') as f:
            ...
            f.write(";\n")
            f.write("var report_time='%s'" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            f.write(";\n")
