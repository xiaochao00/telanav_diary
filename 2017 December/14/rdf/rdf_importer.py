...
class LoadOption(object):
    ...
    SRC_NAME_SENSITIVE_BOUNDARY_RIVER = ['sensitive_boundary_river']
    ...
    def __init__(self, sources=None):
        ...
        self.load_sensitive_boundary_river = self._check_options(sources, self.SRC_NAME_SENSITIVE_BOUNDARY_RIVER)
...
class Importer(object):
    ...
    SENSITIVE_BOUNDARY_RIVER_TABLE = "usr.sensitive_boundary_river"
    SENSITIVE_BOUNDARY_RIVER_SQL = "config/sensitive_boundary_river.sql"

    CMD_IMPORT_SQL = 'psql %s -v ON_ERROR_STOP=1 -f %s'
    ...
    def __del__(self):
        if self.archived_dir and os.path.exists(self.archived_dir):
            shutil.rmtree(self.archived_dir)
    ...
    def import_all(self):
        ...
        if not self._improt_sensitive_boundary_river():
            return False
    ...
    def _pre_process(self):
        ...
        # Pre Processor
        installer = self._get_rdf_installer()
        self.archived_dir = tempfile.mkdtemp(dir=self.data_dir, prefix=self.ARCHIVE)
        self.pre_processor = PreProcessor(self.data_dir, self.archived_dir, region, installer)
    ...
    def _import_rdf(self):
        ...
        #         return False
        for rdf_data_add in self.pre_processor.get_rdf_dir_additional():
            options.mode = 'merge'
            if not Importer._import_rdf_imp(rdf_data_add, rdf_installer, adaptor, options):
                return False
    ...
    def _improt_sensitive_boundary_river(self):
        """
        Import sensitive boundary river data from shape file
            shape files are stored in "components/sensitive_boundary_river"
        If shape files are not exist, then import data from config/sensitive_boundary_river.sql
        :return: True/False
        """

        # check if need to load sensitive boundary river by options.source_list
        if not self.src_options.load_sensitive_boundary_river:
            return True

        logging.info('Importing sensitive boundary river .......')
        # get shape directory
        shape_dir = os.path.join(self.data_dir, Importer.DT_SENSITIVE_BOUNDARY_RIVER)
        # get shape importer
        shape_importer = import_shape.ShapeImporter(self.option, self.importer)
        # import shp file to database
        if not shape_importer.import_shape(shape_dir, Importer.SENSITIVE_BOUNDARY_RIVER_TABLE):
            return False

        # connect to database to check if usr.sensitive_boundary_river is exist
        pgconn = pgconnect.PgConnect(self.option)
        if not pgconn.init_db():
            logging.error('Can not connect to postgis when loading camera\n')
            return False

        if not pgconn.exist(Importer.SENSITIVE_BOUNDARY_RIVER_TABLE):
            # import data from sql file when the table is not exist after import shape
            sql_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), Importer.SENSITIVE_BOUNDARY_RIVER_SQL)
            cmd = Importer.CMD_IMPORT_SQL % (self._format_psql_args(), sql_file)
            logging.info(cmd)
            # execute cmd
            result = os.system(cmd)
            if result:
                logging.error('Importing sensitive boundary from sql file failed! SQL file: %s\n' % sql_file)
                return False

        # add buffer to usr.sensitive_boundary_river.geom
        sqls = [
            # create new table based on usr.sensitive_boundary_river.geom
            "CREATE TABLE %s_buffer AS SELECT gid, ST_Buffer(geom, 0.00005) AS geom FROM %s" % (
                Importer.SENSITIVE_BOUNDARY_RIVER_TABLE, Importer.SENSITIVE_BOUNDARY_RIVER_TABLE),
            # drop old table
            "DROP TABLE %s" % Importer.SENSITIVE_BOUNDARY_RIVER_TABLE,
            # rename new table
            "ALTER TABLE %s_buffer rename to sensitive_boundary_river" % Importer.SENSITIVE_BOUNDARY_RIVER_TABLE,
            # create index for new table
            "CREATE INDEX sensitive_boundary_river_geom_idx ON %s USING gist(geom)" % Importer.SENSITIVE_BOUNDARY_RIVER_TABLE,
            ]
        # if add buffer failure then return false with error log
        if not pgconn.execute(';'.join(sqls)):
            logging.error('Failed to add buffer to sensitive boundary river!')
            return False

        logging.info('Importing sensitive boundary river done!')
