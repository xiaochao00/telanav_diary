...
class DBModel:
    ...
    def init_db(self):
        ...
        try:
            ...
            print_error("Error: connection to db %s : %s failed. check need" % (self.host, self.dbname))
            print e
            sys.exit(-1)
    ...
    def execute_sql(self, sql, is_exist=True):
        ...
        try:
            ...
            self.conn.rollback()
            print_error("Failed: failed execute sql [%s]" % sql)
            print_error(e)
            if is_exist:
                self.close()
                sys.exit(-1)
    ...
    def get_data_basedirectory(self):
        ...
        print_error('Error: get data base directory failed. need checked ')
        return None
