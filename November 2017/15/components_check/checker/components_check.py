...
RDF_ROOT_PATH = os.path.dirname(os.path.dirname(__file__))

sys.path.append(RDF_ROOT_PATH)
from rdf_importer import Importer

CONFIG_FILE_RELATIVE_PATH = 'config/region_components.json'
region_component_conf = os.path.join(RDF_ROOT_PATH, CONFIG_FILE_RELATIVE_PATH)
...
class ExtendChecker(object):
    ...
    def check_all_region(self, to_path=os.path.dirname(__file__)):
        ...
        :param to_path: path to save the report
        :return:
        """
        ...
        for region_path in region_path_list:
            # 2.check every components of this region
            ...
            if region and version:
                ...
                if is_level0:
                    region = "_".join([region, Util.LEVEL0_FLAG])
                # 3.collection success check info and failed check info
                detail_info_list = []
                for failed_component_name, failed_msg in failed_components_check_info_dic.iteritems():
                    detail_info_list.append(":".join([failed_component_name, failed_msg]))
                for component_name, msg in success_components_check_info_dic.iteritems():
                    detail_info_list.append(component_name)
                check_state_str = "Failed" if failed_components_check_info_dic else "Pass"

                check_info_list.append(CheckStateInfo(region, version, detail_info_list, check_state_str))
