...
AXF_EXTERNAL_PATH = os.path.join(ROOT, 'axf/axf_external.cfg')
AXF_SQL_FILE_PATH = os.path.join(ROOT, "axf/sql")
AXF_POST_SQL_FILE_PATH = os.path.join(ROOT, "axf/post_sql")
...
class AxfImporter(object):
    ...
    def _init_pgsql(self):
        ...
        db_args = "host=%s port=%s user=%s dbname=%s" % (options.host, options.port, options.user, "postgres")
        try:
    ...
    def _create_database(self):
        # create db
        database_name = self.options.dbname
        tablespace = self.options.tablespace
        search_db_sql = "SELECT datname FROM pg_database WHERE datname = '%s' " % database_name
        self.cursor.execute(search_db_sql)
        rows = self.cursor.fetchall()

        if not rows:
            # change the isolation first, then can create database
            self.conn.set_isolation_level(0)
            create_sql = " CREATE DATABASE  %s " % self.options.dbname
            if tablespace:
                create_sql += " TABLESPACE %s " % tablespace
            self.cursor.execute(create_sql)
            self.conn.commit()
            self.conn.close()

        db_args = "host=%s port=%s user=%s dbname=%s" % (self.options.host, self.options.port, self.options.user, database_name)
        self.conn = psycopg2.connect(db_args)
        self.cursor = self.conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    def _init_cfg_ex(self):
        ...
        if m:
            ...
            axf_info("write version to config file [%s]" % AXF_EXTERNAL_PATH)
            with open(AXF_EXTERNAL_PATH, 'wb') as f:
                cp.write(f)
        else:
    ...
    def _traffic_export(self):
        ...
        return traffic_export.export_traffic(self.options.schema, self.cursor, self.options.traffic_export_path)
            ...
            if under_mesh:
                ...
                sqls.extend(
                    ['ALTER TABLE %s.%s ALTER COLUMN %s TYPE numeric(18,0)' % (schema, table, field) for field in
                     fields])
                sqls.extend(
                    ['ALTER TABLE %s.%s ALTER COLUMN %s TYPE numeric(18,0)' % (schema, table, col) for col in
                     sp_fields])
                self._execute_sql(sqls)
    ...
    def _import_mask_mapping(self):
        ...
        sql_files = glob.glob(os.path.join(AXF_SQL_FILE_PATH, '*.sql'))
    ...
    def _import_post_sql(self):
        ...
        sql_files = glob.glob(os.path.join(AXF_POST_SQL_FILE_PATH, '*.sql'))
        for sql_file in sql_files:
            cmd = """%s -v ON_ERROR_STOP=1 -q %s -f %s """ % (psql, db_args, sql_file)
            if not execute(cmd):
                axf_error('import mask mapping failed, cmd=[%s]' % cmd)
...
def main():
    ...
    ##
    parser.add_option('-O', '--tollcost-outpath', help='tollcost output path  ', dest='tollcost_outpath')
    parser.add_option('-E', '--traffic-export-path', help='traffic export path  ', dest='traffic_export_path')
