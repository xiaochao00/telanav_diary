...
("gate_type", True, "gate_type")
)

class RelationsGates(Record):
...
WHERE rc.condition_type='4' AND rnl.iso_country_code IN (%s)"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
    ...
    def __statistic(self,line):
        for keys in STATISTIC_KEYS:
            try:
                getattr(self,'_RelationsGates__get_'+keys[2])(keys,line)
            except:
                print "The statistic [ %s ] didn't exist"%(keys[2])
                print ("Unexpected error:[ RelationsGates.py->__statistic] "+str(sys.exc_info()))
    ...
    def __get_barrier(self,keys,line):
        if '4' == line[1]:
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%('gate') or ""))

    def __get_gate_type(self,keys,line):
        gatetype = None
        if '1' == line[2]:
            gatetype = 'Key Required'
        elif '2' == line[2]:
            gatetype = 'Permission Required'
        elif '3' == line[2]:
            gatetype = 'Emergency Vehicle Access'
        if None != gatetype:
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%(gatetype) or ""))

if __name__ == "__main__":
    # use to test this model
    bg = datetime.datetime.now()
    stat =  RelationsGates('na').get_statistic()
    keys = stat.keys()
    print "==>"
