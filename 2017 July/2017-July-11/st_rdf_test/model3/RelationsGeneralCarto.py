...
class RelationsGeneralCarto(Record):
    ...
    def dump2file(self):
        cmd = "SELECT \
            ...
            ORDER BY carto_id"
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self.carto_dump_file,"w"))
    ...
    def dumpcartocandidator2file(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self.carto_candidator_dump_file,"w"))
        print 'dump candidator carto id'

        processcount = 0
        with open(self.carto_candidator_dump_file, 'r', 1024*1024*100) as carto_candidator_f:
            lines = csv.reader(carto_candidator_f, delimiter='`')
            for line in lines:
    ...
    def dumpcartowithmface2file(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self.cartoid_mface_dump_file,"w"))
    ...
    def loadcartoidpolygonclipper(self):
        ...
        with open(self.cartoid_polygoncliiper_cfg, "r", 1024*1024) as cfg:
            lines = csv.reader(cfg, delimiter='`')
            for line in lines:
                line_p = [x.strip() for x in line]
            # for line in cfg:
            #     line = line.strip()
