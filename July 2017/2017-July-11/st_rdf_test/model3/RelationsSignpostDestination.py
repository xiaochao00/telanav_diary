...
class RelationsSignpostDestination(Record):
    ...
    def dump2file(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self.signpost_dump_file,"w"))

    def dumpadminhierarchyfile(self):
        cmd = "select * from rdf_admin_hierarchy"
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV " % (cmd), open(self.admin_hierarchy_dump_file, "w"))
