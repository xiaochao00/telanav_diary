...
class WaysDST(Record):
    ...
    def _dump_way(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self._ways_dump_file,"w"))
    ...
    def _dump_admin_hierarchy(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self._admin_hierary_dump_file,"w"))
        with open(self._admin_hierary_dump_file, 'r', 1024*1024*100) as admin_hierarchy_f:
            lines = csv.reader(admin_hierarchy_f, delimiter='`')
            for line in lines:
                if len(line) == 0:
                    continue
    ...
    def _dump_admin_dst(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '`' CSV "%(cmd),open(self._admin_dst_dump_file,"w"))

        self._load_dst_config()
        with open(self._admin_dst_dump_file, 'r', 1024*1024*10) as dst_f:
            lines = csv.reader(dst_f, delimiter='`')
            for line in lines:
                if len(line) == 0:
                    continue
                line_p = [x.strip() for x in line]
