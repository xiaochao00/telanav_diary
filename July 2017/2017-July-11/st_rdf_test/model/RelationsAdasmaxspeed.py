...
WHERE rc.condition_type='10' AND rnl.iso_country_code IN (%s)"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
    ...
    def __get_adas_special_maxspeed_forward(self,keys,line):
        if '1' == str(line[6]) and '\N' != line[2] and '\N' != line[2] and '0' != str(line[2]):
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_adas_special_maxspeed_backward(self,keys,line):
        if '2' == str(line[6]) and None != line[2] and '\N' != line[2] and '0' != str(line[2]):
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_adas_special_maxspeed(self,keys,line):
        if '1' != str(line[6]) and '2' != str(line[6]) and None != line[2] and  '\N' != line[2] and '0' != str(line[2]):
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_special_speed_type(self,keys,line):
        mp = {'1':'1','2':'2'}
        if str(line[3]) in mp:
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%(mp.get(str(line[3]))) or ""))
    ...
    def __get_depdendent_speed_type(self,keys,line):
        ...
        if str(line[4]) in mp:
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%(mp.get(str(line[4]))) or ""))

    def __get_time_override(self,keys,line):
        mp = {'1':'DAWN TO DUSK','2':'DUSK TO DAWN'}
        if str(line[5]) in mp:
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%(mp.get(str(line[5]))) or ""))
