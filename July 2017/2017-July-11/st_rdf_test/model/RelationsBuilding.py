...
class RelationsBuilding(Record):
    ...
    def dumpenbuilding2file(self):
        ...
        self.run_sql(self.cursor.execute, cmd)
        self.enbuilding_rows = self.cursor.fetchall()
        ...
        #     print "\rProcess index [ "+str(processcount)+" ]",
        for row in self.enbuilding_rows:
            if not row:
                continue
            if 'Y' == row[7]:
                self.enbuilding_main_feature_type[row[1]] = None
            processcount += 1
            if processcount % 5000 == 0:
                print "\rProcess index [ " + str(processcount) + " ]",
        print "\rProcess index [ " + str(processcount) + " ]",
    ...
    def get_statistic(self):
        ...
        for en_row in self.enbuilding_rows:
            if len(en_row) < 1:
                continue
            if self.enbuilding_main_feature_type.has_key(en_row[1]):
                if 'Y' == en_row[7]:
                    self.__statistic(en_row)
                    processed_enbuilding[en_row[1]] = None
            else:
                if not processed_enbuilding.has_key(en_row[1]):
                    self.__statistic(en_row)
                    processed_enbuilding[en_row[1]] = None
