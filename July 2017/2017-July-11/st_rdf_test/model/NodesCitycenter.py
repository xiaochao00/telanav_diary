...
WHERE rcp.iso_country_code IN (%s);"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
        ...
        #self.rows = self.cursor.execute(cmd).fetchall()
    ...
    def get_statistic(self):
        try:
            ...
            return {}
        processcount = 0
        # edit 2017-07-10 by Shichao begin
        ...
        # end by Shichao
        # write to file
        with open(os.path.join(ROOT_DIR, "output", "stat", self.__class__.__name__ + '_2'), 'w') as stf:
    ...
    def __get_place(self,keys,line):
        ...
        if '4444' == str(line[1]):
            place = 'city'
        elif '9709' == str(line[1]):
            place = 'neighbourhood'
        elif '9998' == str(line[1]):
            place = 'hamlet'
        if None != place:
    ...
    def __get_population(self,keys,line):
        ...
        if '\N' != line[2] and '0' != str(line[2]) and None != line[2]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_street_name(self,keys,line):
        if '\N' != line[3] and None != line[3]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_iso(self,keys,line):
        if '\N' != line[4] and None != line[4]:
            iso = line[4]
            if 'A' == line[17] and '\N' != line[19] and None != line[19]:
                iso = line[19]
            self.__count("%s%s%s"%(GLOBAL_KEY_PREFIX,keys[0],keys[1] and "#%s"%(iso) or ""))

    def __get_postal_code(self,keys,line):
        if '\N' != line[5] and None != line[5]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))
    ...
    def __get_claimed_by(self,keys,line):
        if '\N' != line[11] and line[11] != None:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_controlled_by(self,keys,line):
        if '\N' != line[12] and line[12] != None:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_location_score(self,keys,line):
        if '\N' != line[13] and None != line[13]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_place_score(self,keys,line):
        if '\N' != line[14] and None != line[14] :
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_calculated_level(self,keys,line):
        if '\N' != line[15] and None != line[15]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __get_link_count(self,keys,line):
        if '\N' != line[18] and self.__digital_compare(line[18],'>','0') and None != line[18]:
            self.__count("%s%s"%(GLOBAL_KEY_PREFIX,keys[0]))

    def __digital_compare(self, i1, comp, i2):
        if type(i1)=="str" and (not (i1.isdigit() and i2.isdigit())):
            return False
        try:
