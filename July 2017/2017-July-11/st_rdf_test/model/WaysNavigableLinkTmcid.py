#-------------------------------------------------------------------------------
# Name:        WaysNavigableLinkTmcid model
...
STATISTIC_KEYS    = (("tmcid",False,"tmcid"),)

class WaysNavigableLinkTmcid(Record):
...
WHERE rnl.iso_country_code IN (%s)"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
    ...
    def __statistic(self,line):
        for keys in STATISTIC_KEYS:
            try:
                getattr(self,'_WaysNavigableLinkTmcid__get_'+keys[2])(keys,line)
            except:
                print "The statistic [ %s ] didn't exist"%(keys[2])
                print ("Unexpected error:[ WaysNavigableLinkTmcid.py->__statistic] "+str(sys.exc_info()))
...
if __name__ == "__main__":
    ...
    stat =  WaysNavigableLinkTmcid('na').get_statistic()
    keys = stat.keys()
    print "==>"
