...
class RelationsSignpostElement(Record):
    ...
    def dump2file(self):
        ...
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '%s'"%(cmd, CSV_SEP),open(self.signpost_dump_file,"w"))

    def dumpadminhierarchyfile(self):
        cmd = "select * from rdf_admin_hierarchy"
        print cmd
        self.cursor.copy_expert("COPY (%s) TO STDOUT DELIMITER '%s'" % (cmd, CSV_SEP), open(self.admin_hierarchy_dump_file, "w"))
