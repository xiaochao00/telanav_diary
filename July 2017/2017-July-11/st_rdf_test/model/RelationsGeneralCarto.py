...
class RelationsGeneralCarto(Record):
    ...
    def dumpcartocandidator2file(self):
        ...
        self.run_sql(self.cursor.execute, cmd)
        carto_candidator_rows = self.cursor.fetchall()
        print 'dump candidator carto id'

        processcount = 0
    # with open(self.carto_candidator_dump_file, 'r', 1024*1024*100) as carto_candidator_f:
        for carto_candidator_row in carto_candidator_rows:
            if not carto_candidator_row[0]:
                continue
            self.carto_candidator[carto_candidator_row[0]] = None
            processcount += 1
    ...
    def dumpcartowithmface2file(self):
        ...
        self.run_sql(self.cursor.execute, cmd)
        cartoid_mface_rows = self.cursor.fetchall()

        #print "Dump carto id with multiple faces"
        processcount = 0

        for cartoid_mface_row in cartoid_mface_rows:
            if not cartoid_mface_row[0]:
                continue
            self.cartoidmface[cartoid_mface_row[0]] = None
            processcount += 1
            if processcount%5000 == 0:
                print "\rProcess index [ "+str(processcount)+" ]",
        print "\rProcess index [ "+str(processcount)+" ]",
    ...
    def get_statistic(self):
        ...
        for row in self.rows:
            if len(row) < 1:
                continue
            if not self.carto_candidator.has_key(row[1]) and str(row[2]) != '500116':
                continue
    ...
    def __get_featuretype(self, line):
        ...
        if line[2]==900103:
            pass
        if FEATURE_TYPE_MAPPING.has_key(str(line[2])):
            vals = FEATURE_TYPE_MAPPING[str(line[2])]
            #900103
            for val in vals:
