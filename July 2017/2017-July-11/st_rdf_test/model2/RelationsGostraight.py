#-------------------------------------------------------------------------------
# Name:        RelationsGostraight model
...
class RelationsGostraight(Record):
...
WHERE rc.condition_type='14' AND rnl.iso_country_code IN (%s)"%(REGION_COUNTRY_CODES(self.region, GLOBAL_KEY_PREFIX))
    ...
    def __statistic(self,line):
        for keys in STATISTIC_KEYS:
            try:
                getattr(self,'_RelationsGostraight__get_'+keys[2])(keys,line)
            except:
                print "The statistic [ %s ] didn't exist"%(keys[2])
                print ("Unexpected error:[ RelationsGostraight.py->__statistic] "+str(sys.exc_info()))
...
if __name__ == "__main__":
    ...
    stat =  RelationsGostraight('na').get_statistic()
    keys = stat.keys()
    print "==>"
